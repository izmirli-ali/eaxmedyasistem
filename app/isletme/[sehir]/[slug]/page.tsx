...
interface Props {
  params: { sehir: string; slug: string }
}
...
export default function IsletmeDetayClient({ params }: Props) {
  ...
  const fetchIsletme = useCallback(async () => {
    try {
      ...
      const { data, error } = await supabase
        .from("isletmeler")
        .select("*")
        .eq("url_slug", `${params.sehir}/${params.slug}`)
        .single()
  ...
  }, [params.sehir, params.slug, supabase])
  ...
  const getPageUrl = useCallback(() => {
    ...
    return `${process.env.NEXT_PUBLIC_SITE_URL || "https://isletmenionecikar.com"}/isletme/${params.sehir}/${params.slug}`
  }, [params.sehir, params.slug])
