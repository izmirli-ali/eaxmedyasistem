...
export default function IsletmeDetayClient({ params }: Props) {
  ...
  const fetchIsletme = useCallback(async () => {
    try {
      ...
      const { data, error } = await supabase.from("isletmeler").select("*").eq("url_slug", `${params.sehir}/${params.slug}`).single()
  ...
  const fetchSiteSettings = useCallback(async () => {
    try {
      ...
      // Site ayarları hatası kritik değil, ana hatayı etkilemesin
    }
  }, [supabase])
  ...
  useEffect(() => {
    async function fetchData() {
      await Promise.all([fetchIsletme(), fetchSiteSettings()])
    }
  ...
  // Site ayarları yüklenemedi ama işletme yüklendiyse devam et
  if (!siteSettings) {
    console.warn("Site ayarları yüklenemedi, varsayılan değerler kullanılacak")
  }
