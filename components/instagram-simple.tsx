    ...
    const loadInstagramEmbed = async () => {
      try {
        ...
        if (!document.getElementById("instagram-embed-script")) {
          ...
          script.onload = () => {
            // @ts-ignore - Instagram global değişkeni
            if (window.instgrm) {
              // @ts-ignore
          ...
          // Script zaten yüklenmişse, embeds'i yeniden işle
          // @ts-ignore
          if (window.instgrm) {
            // @ts-ignore
            window.instgrm.Embeds.process()
        ...
        console.error("Instagram embed yüklenirken hata:", err)
    ...
    return (
      <InfoCard>
        <div className="text-center py-6">
          <p className="text-gray-500">{error || "Instagram bilgisi bulunamadı"}</p>
        </div>
      </InfoCard>
    )
  }
